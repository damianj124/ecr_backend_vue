<template>
  <li
    :class="[isFolder?'ecrw-ready-left-li':'ecrw-ready-left-li-last', selected?'ecrw-ready-left-li-selected':'']"
  >
    <span class="item-content">
      <span
        class="toggle"
        :class="{'ecrw-ready-left-list-toggle': isFolder&&!isOpen}"
        v-show="isFolder"
        @click.stop="toggle()"
      ></span>
      <a
        class="title"
        href="javascript:;"
        :class="{ 'highlight':highlight , 'bigtype': model.parent!=null&&model.parent.parent==null }"
      ...
      <span class="ecrm-tab-level-col" v-if="tools.length>0 && level>=toolMinLevel">
        <a
          href="javascript:;"
          class="ecrm-tabl-level-ico"
          v-if="level>=tool.level"
          :class="tool.class"
      ...
      <!-- <a
        href="javascript:;"
        v-if="(model.children.length==this.pageSize)&&(count[model.children[0].themeData.parentthemeid] != pageCount[model.children[0].themeData.parentthemeid])"
        @click="setInc(model.children[0].themeData.parentthemeid)"
      ...
      <a
        ...
        @click="setDec(model.children[0].themeData.parentthemeid)"
      ></a> -->
    </span>
    <!-- // Author : GOD
...
export default {
  ...
  computed: {
    isFolder: function() {
      return (
        this.model.themeData != null && this.model.themeData.childCount > 0
      );
    }
  },
  created() {
    ...
    // Reason : 系统管理 专题管理 Page Loading Speed, Select First Theme as Default
    if (
      this.model.themeData &&
      this.model.themeData.themeid &&
      this.themeid == this.model.themeData.themeid
    )
      this.selected = true;
  ...
  methods: {
    setInc(parentid) {
      ...
      if (this.count[parentid] > 1) this.showBeforeBu = true;
    },
    setDec(parentid) {
      this.count[parentid]--;
      this.toggleHandle(this.count[parentid], parentid);
      if (this.count[parentid] == 1) this.showBeforeBu = false;
    },
    toggle: function() {
      let para1 = {
        // GOD 2019-2-25 Bug ID: #904
        pagination: {
          // numPerPage: 10
        },
        // GOD 2019-2-25 Bug ID: #904
        conditions: [
          {
            ...
            fieldValues: [this.model.themeData.themeid],
            prepender: null,
            operator: "EQUAL",
      ...
      getThemeList(para1).then(res => {
        if (res.data.serverResult.resultCode == "200") {
          ...
          this.pageCount[
            this.model.children[0].themeData.parentthemeid
    ...
    doClick: function() {
      ...
      if (this.selected) this.$emit("item-select", this, this.model);
      else this.$emit("item-deselect", this, this.model);
    },
    doChange() {
      this.selected = !this.selected;
    },
...
.ecrm-tabl-level-edit {
  background: url("../../../../static/img/ecrm-tag-edit.png") no-repeat;
}
.ecrm-tabl-level-add {
  background: url("../../../../static/img/ecrm-tag-add.png") no-repeat;
}
.ecrm-tabl-level-sub {
  background: url("../../../../static/img/ecrm-tag-sub.png") no-repeat;
}
.ecrm-tabl-level-refresh {
  background: url("../../../../static/img/ecrm-tag-refresh.png") no-repeat;
}
.ecrm-tabl-level-move {
  background: url("../../../../static/img/ecrm-tag-move.png") no-repeat;
}
.ecrm-tabl-level-edit:hover {
  background: url("../../../../static/img/ecrm-tag-edit-sel.png") no-repeat;
}
.ecrm-tabl-level-add:hover {
  background: url("../../../../static/img/ecrm-tag-add-sel.png") no-repeat;
}
.ecrm-tabl-level-sub:hover {
  background: url("../../../../static/img/ecrm-tag-sub-sel.png") no-repeat;
}
.ecrm-tabl-level-refresh:hover {
  background: url("../../../../static/img/ecrm-tag-refresh-sel.png") no-repeat;
}
.ecrm-tabl-level-move:hover {
  background: url("../../../../static/img/ecrm-tag-move-sel.png") no-repeat;
}
.ecrm-dialog-icon-up {
