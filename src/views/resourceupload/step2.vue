<template>
    <el-row>
        <el-col :span="24" align="center" v-for="(form,idx) in forms" :key="'percent'+idx" :data="form" v-if="percentVisible[idx]">
        ...
        <el-col :span="24" align="right">
            <el-form :inline="true" class="ecrm-upload-second">
                <el-form-item>
        ...
        <el-form ref="form" :model="form" v-for="(form, frmIdx) in forms" :rules="formRules" :key="frmIdx" :data="form" label-width="90px" style="margin:20px;" class="ecrm-upload-resource">
            <el-form-item label="资源标题 :" prop="name">
                ...
                <el-col :span="8" class="ecrm-upload-resource-desc">
                    备注：上传文件后，自动读取文件名。
            ...
            <el-form-item :label="cptIdx==0?'所属章节 :':''" v-for="(chapter, cptIdx) in form.chapters" :key="cptIdx" :data="chapter">
                ...
                <el-col :span="6" v-if="cptIdx==0" style="padding-left: 15px;">
...
<script>
    ...
    import { getSubjectList, getSchoolSectionList, getNaviGradeList,getNaviEditionTypeList, getTeachCatalogList,getNaviBookModelList,uploadResource, getCatalogHeader, getCatalogList, uploadFile, uploadFileInfo } from '../../api/api';
        ...
        methods: {
            validateNameProperty(rule, value, callback) {
                ...
                if (filename.length > 25) {
                    callback(new Error('文件名不能超过25个字'));
            ...
            load() {
                ...
                if(localStorage.getItem('backend-user')){
                    this.userClassInfo  = JSON.parse(localStorage.getItem('backend-user')).classinfo;
                } else {
                    this.userClassInfo = [];
                }   
            },
            ...
            uploadFile: function(contentId, index) {
                ...
                this.percentVisible[index] = true;
                var _this = this;
                uploadFile({ data:data, onUploadProgress:function(progressEvent) {
                    _this.uploadProgress[index] = Math.floor(progressEvent.loaded * 100 / progressEvent.total);
                }}).then((res) => {
...
<style scoped lang="scss">
    ...
    .ecrm-table-left-list-close {
        background: url('../../../static/img/ecrm-icon-open-close.png') no-repeat left center;
        display: inline-block;
        cursor: pointer;
        width: 18px;
        height: 11px;
