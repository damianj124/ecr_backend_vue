    ...
    <section>
        ...
        <el-col :span="24" class="ecrm-content-panel" style="max-width:1080px;min-height:300px" v-loading="editLoading">
            <el-form ref="resourceForm" :model="resourceInfo" :rules="formRules" label-width="120px" v-if="resourceInfo != null">
                    ...
                    <el-row>
                        <el-col :span="14">
                            <el-form-item label="资源文件:">
                                <el-button v-if="resourceInfo.fileInfo!=null"  @click="resetFile" :loading="uploadingFile">修改资源文件</el-button>
                            </el-form-item>
                        </el-col>
                        <el-col :span="7">
                ...
                <div style="background: #ededed; padding: 5px 20px; width: 60px; margin-top: 15px" v-if="resourceInfo.status=='3' || resultFlag">
                    <span>
                        扩展属性
                    </span>
                </div>
                <div style="padding: 0px 0px 10px 0px; border-bottom: 1px dashed #e8e8e8;"  v-if="resourceInfo.status=='3' || resultFlag">
                    <el-row>
                        <el-col :span="21">
                ...
                <div style="padding: 0px 0px 10px 0px; border-bottom: 1px dashed #e8e8e8;">
                    <el-row>
                        <el-col :span="21">
                            <el-form-item label="审核结论:">
                                <el-radio-group v-model="auditinfo.resulttext" @change="onChangeResult(auditinfo.resulttext)">
                    ...
                    <el-row>
                        <el-col :span="21">
                            <el-form-item label="审核意见:">
                                <el-input type="textarea" :rows.native="5" v-model="auditinfo.remarktext"></el-input>
                            </el-form-item>
                        </el-col>
                    </el-row>
                </div>
...
<script>
    ...
    import { CONTENT_TYPE, SHARE_RANGE, REVIEW_STATUS, ALL_EXTENSIONS } from '../../common/js/const.js';
    import { getSchoolSectionList, getSubjectList, getNaviBookModelList, getNaviEditionTypeList, getNaviGradeList, detailResource, getTeachCatalogList, getOneLabelList, getTwoLabelList, getThreeLabelList, editResource, uploadFile, uploadFileInfo } from '../../api/api';
    ...
    export default {
        data() {
            return {
                ...
                formRules: {
                    ...
                    checkstuffdate: [
                        { validator: this.validatestuffDate }
                    ]
                },
        ...
        methods: {
            validatestuffDate(rule, value, callback) {
                ...
                console.log(this.goodinfo.onshelftime);
                console.log(this.goodinfo.downshelftime);
                if (this.goodinfo.onshelftime == '') {
                    callback(new Error('请输入开始时间'));
                } else if (this.goodinfo.downshelftime == '') {
                    callback(new Error('请输入结束时间'));
                } else if(new Date(this.goodinfo.onshelftime) > new Date(this.goodinfo.downshelftime)){
                    callback(new Error('查询结束时间不得大于开始时间。'));
                } else if(ECRUtil.formatDate.format(new Date(this.goodinfo.onshelftime), 'yyyy-MM-dd') == ECRUtil.formatDate.format(new Date(this.goodinfo.downshelftime), 'yyyy-MM-dd')){
            ...
            load() {
                ...
                detailResource(resourceId).then((res) => {
                    if (res.data.serverResult.resultCode == "200") {
                        ...
                        if (this.resourceInfo.status == "3") {
                            this.auditinfo.resulttext = "通过";
                        } else if(this.resourceInfo.status == "4") {
                            this.auditinfo.resulttext = "不通过";
                        } else {
                            this.auditinfo.resulttext = "";
                        }

                        if (this.resourceInfo.auditinfo != null) {
                            this.auditinfo.auditid = this.resourceInfo.auditinfo.auditid;
                            this.auditinfo.remarktext = this.resourceInfo.auditinfo.remark;
                        }
            ...
            editSubmit() {
                this.$refs.resourceForm.validate((valid) => {
                      if (valid) {
                        ...
                        if (this.auditinfo.resulttext == "通过") {
                            isresult = "1";
                            this.resourceInfo.status = "3";
                        } else if (this.auditinfo.resulttext == "不通过") {
                            isresult = "0";
                            this.resourceInfo.status = "4";
                        }
                        ...
                        let params = {
                            ...
                            twolabelid: (this.resourceInfo.twolabelid)? this.resourceInfo.twolabelid: null,
                            ...
                            auditinfo: {
                                auditid: this.auditinfo.auditid,
                                result: isresult,
                                remark: this.auditinfo.remarktext
                            }
                        };
            ...
            chosenFile: function(e) {
                ...
                for (var i = 0; i < this.format.length; i++) {
                    if(fileFormat == this.format[i]) {
                        this.uploadFile(this.resourceInfo.contentid, file);
                        this.filterContentTypeWithFormat(fileFormat);
                        return;
                    } 
                }
                this.$message({
            ...
            filterContentTypeWithFormat(format) {
                this.contentTypes = ECRUtil.getContentTypesOfExt(format);
                for (var i=0; i<this.contentTypes.length; i++) {
                    if (this.contentTypes.contenttype == this.resourceInfo.contenttype) {
                        return;
                    }
                }
                if (this.contentTypes.length > 0) {
                    this.resourceInfo.contenttype = this.contentTypes[0].contenttype;
                }
            },
            uploadFile(contentId, file) {
                ...
                uploadFile({ data:data, onUploadProgress:0 }).then((res) => {
                    if (res.data.serverReults.resultCode == "200") {
                        let fileInfo = {
                                fileid:res.data.fileInfos[0].fileId,
                                filename:res.data.fileInfos[0].fileName,
                                size:res.data.fileInfos[0].size,
                                localpath:res.data.fileInfos[0].localPath,
                                format:res.data.fileInfos[0].format,
                                file_md5:res.data.fileInfos[0].md5,
                                createtime:res.data.fileInfos[0].createTime,
                                lastmodifytime:res.data.fileInfos[0].lastModifyTime
                            };
                        this.resourceInfo.fileInfo = fileInfo;
                    } else {
                        this.$message({
