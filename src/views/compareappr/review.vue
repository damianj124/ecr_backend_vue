    ...
    <section>
        ...
        <el-col :span="24" class="ecrm-content-panel">
            ...
            <el-form label-width="100px">
                ...
                <el-col :span="6">
                    <el-form-item label="搜索框:">
                        <el-input v-model="filters.searchkey" placeholder="请输入资源名称 / 作者" style="width:calc(100% - 65px)" @keyup.enter.native="search"></el-input>
            ...
            <el-col :span="24" style="margin-bottom:10px;text-align:right;">
                <el-button type="table-op" size="small" :disabled="this.sels.length==0" @click="doBatchAllow">批量通过</el-button>
                <el-button type="table-op" size="small" :disabled="this.sels.length==0" @click="doBatchDeny">批量不通过</el-button>
            </el-col>
            <!-- Data Table -->
            ...
            <el-table :data="resources" v-loading="listLoading" @selection-change="selsChange" v-if="resources.length>0">
                ...
                <el-table-column label="审核" align="center">
                    <template slot-scope="resource">
                        <el-select @change="review(resource.$index, resource.row)" v-model="resourceStatus[resource.$index].status">
                ...
                <el-table-column label="活动ID" prop="activityid" align="center">
                </el-table-column>
                ...
                <el-table-column label="操作" align="center" width="150">
                    <template slot-scope="resource">
                        <el-button size="small" @click="doDetail(resource.$index, resource.row)" style="margin:0px">详情</el-button>
                        <el-button size="small" @click="doStuffCreate(resource.$index, resource.row)" style="margin:0px" v-if="resource.row.status=='3' && resource.row.isgoods=='0'">生成</el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog title="审核通过" v-model="allowFormVisible" :close-on-click-modal="false" :show-close="false">
            <el-form :model="allowForm" label-width="100px" ref="allowForm">
                <el-form-item label="积分:" class="is-required">
                    <el-input-number v-model.number="allowForm.collectscore" :min="0" :max="99999"/>
                </el-form-item>
            </el-form>
        ...
        <el-dialog title="审核不通过" v-model="disallowFormVisible" :close-on-click-modal="false" :show-close="false">
            <el-form :model="disallowForm" label-width="100px" ref="disallowForm">
                <el-form-item label="驳回原因:" prop="remark">
                    <el-input type="textarea" :rows.native="8" v-model="disallowForm.remark" auto-complete="off"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                ...
                <el-row>
                    <el-col :span="10">
                        <el-form-item label="价格:" class="is-required" prop="goodprice">
                            <el-input-number v-model="createForm.goodprice" :min="0" :max="99999"></el-input-number>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span="14">
                        <el-form-item label="有效日期:" class="is-required" prop="property">
                            <el-date-picker type="date" v-model="createForm.sell_begindate" style="width:46%;"></el-date-picker>
                            -
                            <el-date-picker type="date" v-model="createForm.sell_enddate" style="width:46%;"></el-date-picker>
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
...
<script>
    ...
    import { getSchoolSectionList, getSubjectList, getNaviGradeList, getNaviBookModelList, getNaviEditionTypeList, getEstimateResourceList, denyResource, allowResource, addBatchGoods  } from '../../api/api';
    ...
    export default {
        data() {
            return {
                ...
                createFormRules: {
                    property: [
                        { validator: this.validateCreateProperty, trigger: 'blur' },
                    ]
                },
        ...
        methods: {
            validateCreateProperty(rule, value, callback) {
                if (this.createForm.sell_begindate == '' || this.createForm.sell_enddate == '') {
...
// Data End : 2019-1-28 12:35 AM
            ...
            review(index, resource) {
                ...
                if (this.resourceStatus[index].status == "3") { //审核
                    this.doAllow(index, resource);
                } else if (this.resourceStatus[index].status == "4") { //驳回
            ...
            cancelDisallow() {
                ...
                this.resourceStatus[this.currentIndex].status = this.currentResource.status;
            },
            cancelAllow() {
            ...
            doAllowSubmit() {
                this.$confirm('是否审核通过该资源？', '提示', {
                    ...
                    let params = {
                        audittype: 1,
                        contentids: this.contentIds,
                        auditscore: parseInt(this.allowForm.collectscore)
                    };
                    allowResource(params).then((res) => {
                        if (res.data.serverResult.resultCode == "200") {
                            for (var i=0; i<this.sels.length; i++) {
                                this.sels[i].status = "4"; //驳回
                            }
            ...
            doBatchAllow() {
                this.allowFormVisible = true;
                this.contentIds = this.sels.map(item => item.contentid).toString().split(",");
            ...
            doStuffCreate(index, resource) {
                ...
                this.createForm.goodprice = resource.score;
                ...
                this.$refs.createForm.validate((valid) => {
                    if (valid) {
                        let para = { 
                            ...
                            sell_begindate: ECRUtil.formatDate.format(new Date(this.createForm.sell_begindate), 'yyyy-MM-dd hh:mm:ss'),
                            sell_enddate: ECRUtil.formatDate.format(new Date(this.createForm.sell_enddate), 'yyyy-MM-dd hh:mm:ss')
                        };
                        this.addLoading = true;
                        addBatchGoods(para).then((res) => {
                            if (res.data.serverResult.resultCode == "200") {
                                this.getResources();
                            } else {
                                this.$message({
