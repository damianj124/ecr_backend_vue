        ...
        <el-col :span="24" class="ecrm-content-panel" style="max-width:1080px;min-height:300px" v-loading="editLoading">
            <el-form ref="resourceForm" :model="resourceInfo" :rules="formRules" label-width="120px" v-if="resourceInfo != null">
                    ...
                    <el-row>
                        <el-col :span="21">
                            <el-form-item label="资源描述:" prop="description">
                                <el-input type="textarea" native:rows.native="5" v-model="resourceInfo.description" auto-complete="off" maxlength.native="100"></el-input>
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row>
                ...
                <div style="padding: 0px 0px 10px 0px; border-bottom: 1px dashed #e8e8e8;">
                    ...
                    <el-row>
                        <el-col :span="21">
                            <el-form-item label="上架下架状态:" @change="onChangeStuffCreation(goodinfo.statustext)">
                                <el-radio-group v-model="goodinfo.statustext">
                    ...
                    <el-row v-if="goodinfo.statustext=='上架'">
                        <el-col :span="21">
                            <el-form-item label="有效日期:" class="is-required" prop="checkstuffdate">
                                <el-date-picker type="date" v-model="resourceInfo.goodinfo.onshelftime"></el-date-picker>
                                -
                                <el-date-picker type="date" v-model="resourceInfo.goodinfo.downshelftime"></el-date-picker>
                            </el-form-item>
                        </el-col>
                    </el-row>
                </div>
    ...
    export default {
        data() {
            return {
                ...
                selectedEditionType:'',
                // Author : GOD
                // Date Start : 2019-2-11 6 PM
                // Reason : Goods Price Increase/Descrease
                goodinfo: {
                    goodid: '',
                    goodprice: 0,
                    goodstatustext: '',
        ...
        methods: {
            ...
            validateDescription(rule, value, callback) {
                ...
                if (description.length > 500) {
                    callback(new Error('不超过500个汉字的长度'));
                } else if (value == "" || value == null) {
            ...
            validatestuffDate(rule, value, callback) {
                ...
                if (this.resourceInfo.goodinfo.onshelftime == '' || this.resourceInfo.goodinfo.downshelftime == '') {
            ...
            onChangeStuffCreation(isstuff) {
              if (isstuff == '下架') {
                    let now = new Date();
                    this.resourceInfo.goodinfo.onshelftime = ECRUtil.formatDate.format(now, 'yyyy-MM-dd hh:mm:ss');
                    now.setTime(now.getTime() + 30 * 24 * 3600 * 1000);
                    this.resourceInfo.goodinfo.downshelftime = ECRUtil.formatDate.format(now, 'yyyy-MM-dd hh:mm:ss');
                }  
            },
            editSubmit() {
                this.$refs.resourceForm.validate((valid) => {
                      if (valid) {
                        ...
                        if (this.goodinfo.statustext == "下架") {
                            ...
                            isstatus = "1";
                            if (this.resourceInfo.goodinfo.onshelftime != '' && this.resourceInfo.goodinfo.downshelftime != '') {
                                if (new Date(this.resourceInfo.goodinfo.onshelftime) > new Date(this.resourceInfo.goodinfo.downshelftime)) {
                                    this.$message({
                                        message: '结束时间不得大于开始时间。',
                                        type: 'error'
                                    });                                   
                                    return;
                                } else if (ECRUtil.formatDate.format(new Date(this.resourceInfo.goodinfo.onshelftime), 'yyyy-MM-dd') == ECRUtil.formatDate.format(new Date(this.resourceInfo.goodinfo.downshelftime), 'yyyy-MM-dd')) {
                                    this.$message({
                        ...
                        let params = {
                            ...
                            goodinfo: {
                                ...
                                status: isstatus,
                                onshelftime: ECRUtil.formatDate.format(new Date(this.resourceInfo.goodinfo.onshelftime), 'yyyy-MM-dd hh:mm:ss'),
                                downshelftime: ECRUtil.formatDate.format(new Date(this.resourceInfo.goodinfo.downshelftime), 'yyyy-MM-dd hh:mm:ss')
                            }
                        };
